<?php
$title = 'Ajouter une expérience - Alumni CNDA';
$content = '
<div class="bg-white rounded-xl shadow-md overflow-hidden">
    <div class="bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-4">
        <h1 class="text-2xl font-bold text-white">➕ Ajouter une expérience professionnelle</h1>
    </div>
    ...
    <div class="p-6">
        ...
        <form method="POST" action="/work-experience/add" class="space-y-5">
            <div class="grid md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    <label for="sCompany" class="block text-sm font-medium text-gray-700 mb-2">Entreprise *</label>
                    <input type="text" id="sCompany" name="sCompany" value="' . htmlspecialchars($data['sCompany'] ?? '') . '" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                </div>
                ...
                <div>
                    ...
                    <select id="iDivision" name="iDivision" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                        ...
                        ' . implode('', array_map(function($div) {
                            return '<option value="' . $div->id . '">' . htmlspecialchars($div->sLabel ?? '') . '</option>';
                        }, $divisions)) . '
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="iCity" name="iCity" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                        ...
                        ' . implode('', array_map(function($city) {
                            return '<option value="' . $city->id . '">' . htmlspecialchars($city->sLabel ?? '') . '</option>';
                        }, $cities)) . '
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="sCity" name="sCity" value="' . htmlspecialchars($data['sCity'] ?? '') . '" placeholder="Ou entrez manuellement une ville" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                </div>

                <div>
                    <label for="dStart" class="block text-sm font-medium text-gray-700 mb-2">Date de début</label>
                    <input type="date" id="dStart" name="dStart" value="' . htmlspecialchars($data['dStart'] ?? '') . '" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                </div>

                <div>
                    <label for="iEnd" class="block text-sm font-medium text-gray-700 mb-2">Année de fin</label>
                    <input type="number" id="iEnd" name="iEnd" min="1900" max="2100" value="' . htmlspecialchars($data['iEnd'] ?? '') . '" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">
                </div>

                <div class="md:col-span-2">
                    <label for="sDescription" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                    <textarea id="sDescription" name="sDescription" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition">' . htmlspecialchars($data['sDescription'] ?? '') . '</textarea>
                </div>
            </div>
